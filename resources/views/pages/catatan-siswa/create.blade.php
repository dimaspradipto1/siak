...
@section('title', 'Tambah Catatan Siswa')

@section('content')
    <div class="pagetitle">
        <h1>Tambah Catatan Siswa</h1>
        <nav>
            <ol class="breadcrumb">
            ...
            <div class="col-lg-10">
                <div class="card shadow-sm">
                    <div class="card-body pt-4">
                        ...
                        <form action="{{ route('catatansiswa.store') }}" method="POST">
                            @csrf
                                ...
                                <div class="col-md-6 mt-3 mt-md-0">
                                    ...
                                    <select id="jenis_catatan_id" name="jenis_catatan_id" class="form-select @error('jenis_catatan_id') is-invalid @enderror">
                                        ...
                                        @foreach($jenisCatatans as $jc)
                                            <option value="{{ $jc->id }}" {{ old('jenis_catatan_id') == $jc->id ? 'selected' : '' }}>{{ $jc->nama_jenis_catatan }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select id="guru_id" name="guru_id" class="form-select select2 @error('guru_id') is-invalid @enderror">
                                        ...
                                        @foreach($gurus as $guru)
                                            <option value="{{ $guru->id }}" {{ old('guru_id') == $guru->id ? 'selected' : '' }}>{{ $guru->pegawai->nama_pegawai ?? '-' }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="isi_catatan" name="isi_catatan" rows="3" class="form-control @error('isi_catatan') is-invalid @enderror">{{ old('isi_catatan') }}</textarea>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea id="tindak_lanjut" name="tindak_lanjut" rows="2" class="form-control @error('tindak_lanjut') is-invalid @enderror">{{ old('tindak_lanjut') }}</textarea>
