...
@section('content')
    ...
    <section class="section">
        <form action="{{ route('materipembelajaran.update', $materipembelajaran->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-lg-8">
                    <div class="card shadow-sm border-0 pb-3">
                        <div class="card-body pt-4">
                            <h5 class="card-title p-0 mb-3 fs-5">Form Edit Data Materi Pembelajaran</h5>

                            <div class="mb-3">
                                <label for="judul_materi" class="form-label fw-semibold">Judul Materi <span class="text-danger">*</span></label>
                                <input type="text" id="judul_materi" name="judul_materi" class="form-control @error('judul_materi') is-invalid @enderror" value="{{ old('judul_materi', $materipembelajaran->judul_materi) }}" placeholder="Masukkan judul materi pelajaran">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea id="deskripsi_materi" name="deskripsi_materi" rows="6" class="form-control @error('deskripsi_materi') is-invalid @enderror" placeholder="Tulis ringkasan atau keterangan materi pelajaran...">{{ old('deskripsi_materi', $materipembelajaran->deskripsi_materi) }}</textarea>
                                @error('deskripsi_materi')<div class="invalid-feedback">{{ $message }}</div>@enderror
                            </div>

                            <div class="mb-0">
                                <label for="file_materi" class="form-label fw-semibold">File Materi</label>
                        ...
                        <div class="card-body pt-4">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="semester_name" name="semester_name" class="form-select @error('semester_name') is-invalid @enderror">
                                    ...
                                    <option value="Semester 1 (Ganjil)" {{ old('semester_name', $materipembelajaran->semester ? $materipembelajaran->semester->nama_semester : '') == 'Semester 1 (Ganjil)' ? 'selected' : '' }}>Semester 1 (Ganjil)</option>
                                    <option value="Semester 2 (Genap)" {{ old('semester_name', $materipembelajaran->semester ? $materipembelajaran->semester->nama_semester : '') == 'Semester 2 (Genap)' ? 'selected' : '' }}>Semester 2 (Genap)</option>
                                </select>
                            ...
                            <div class="mb-0">
                                ...
                                <select id="mata_pelajaran_id" name="mata_pelajaran_id" class="form-select @error('mata_pelajaran_id') is-invalid @enderror">
                                    ...
                                    @foreach($matapelajarans as $mp)
                                        <option value="{{ $mp->id }}" {{ old('mata_pelajaran_id', $materipembelajaran->mata_pelajaran_id) == $mp->id ? 'selected' : '' }}>
                ...
                <div class="col-12 mt-3">
                    <div class="card shadow-sm border-0">
                        <div class="card-body py-3 d-flex justify-content-between align-items-center">
                            ...
                            <button type="submit" class="btn btn-primary"><i class="bi bi-send-fill me-1"></i> Perbarui Data</button>
                        </div>
                    </div>
                </div>
            </div>
