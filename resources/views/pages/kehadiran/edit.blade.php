                        ...
                        <form action="{{ route('kehadiran.update', $kehadiran->id) }}" method="POST">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id="siswa_id" name="siswa_id" class="form-select select2 @error('siswa_id') is-invalid @enderror">
                                        @foreach($siswas as $siswa)
                                            <option value="{{ $siswa->id }}" {{ old('siswa_id', $kehadiran->siswa_id) == $siswa->id ? 'selected' : '' }}>{{ $siswa->nama_siswa }} ({{ $siswa->kelas->nama_kelas ?? '-' }})</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mt-3 mt-md-0">
                                    ...
                                    <select id="mata_pelajaran_id" name="mata_pelajaran_id" class="form-select select2 @error('mata_pelajaran_id') is-invalid @enderror">
                                        @foreach($mapels as $mapel)
                                            <option value="{{ $mapel->id }}" {{ old('mata_pelajaran_id', $kehadiran->mata_pelajaran_id) == $mapel->id ? 'selected' : '' }}>{{ $mapel->nama_mata_pelajaran }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    ...
                                    <input type="date" id="tanggal" name="tanggal" class="form-control @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $kehadiran->tanggal) }}">
                                ...
                                <div class="col-md-6 mt-3 mt-md-0">
                                    ...
                                    <select id="jenis_kehadiran_id" name="jenis_kehadiran_id" class="form-select @error('jenis_kehadiran_id') is-invalid @enderror">
                                        @foreach($jenisKehadirans as $jk)
                                            <option value="{{ $jk->id }}" {{ old('jenis_kehadiran_id', $kehadiran->jenis_kehadiran_id) == $jk->id ? 'selected' : '' }}>{{ $jk->nama_kehadiran }} ({{ $jk->kode_kehadiran }})</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea id="keterangan" name="keterangan" rows="2" class="form-control @error('keterangan') is-invalid @enderror">{{ old('keterangan', $kehadiran->keterangan) }}</textarea>
